...
ALPHABET = set("qwertyuiopasdfghjklzxcvbnm" "QWERTYUIOPASDFGHJKLZXCVBNM1234567890_")
MAX_LABEL_LEN = 128
...
class Simulation:
    def __init__(
        self, grid_side=DEFAULT_GRID_SIDE, debug=False, max_steps=None, grid=None
    ):
            ...
            # Interpret line
                ...
                for op_code in op_map:
                    if line.startswith(op_code):
                        if op_map[op_code] in [OP_JUMP, OP_BORDERJ, OP_PEBBLEJ]:
                            label = line[len(op_code) :].strip()
                            check_label(label)
                        else:
                            if line != op_code:
                                raise ParserException(
                                    "Could not parse " "line %s" % (line)
                                )
                            label = None
    ...
    def compile_runner(self):
        ...
        res = os.system("gcc -O2 -static -Wall -o %s %s" % (self.runfile, self.srcfile))
        if res != 0:
            self.clean_runner()
    ...
    def execute_runner(self):
        ...
        popen.stdin.write(
            bytes(
                "%d %d %d %d %d %d\n"
                % (self.posx, self.posy, self.dir, self.step_num, self.ip, max_steps),
                "utf-8",
            )
        )
        for i in range(self.grid_side):
            for j in range(self.grid_side):
                popen.stdin.write(bytes("%d " % (self.grid[i][j]), "utf-8"))
        ...
        self.posx, self.posy, self.dir, self.step_num, self.ip, self.killed = map(
            int, popen.stdout.readline().decode("utf-8").strip().split(" ")
        )
        self.killed = True if self.killed != 0 else False
        for i in range(self.grid_side):
            self.grid[i] = [
                int(x)
                for x in popen.stdout.readline().decode("utf-8").strip().split(" ")
            ]
    ...
    def eval_move(self):
        ...
        if (
            0 <= new_posx
            and new_posx < self.grid_side
            and 0 <= new_posy
            and new_posy < self.grid_side
        ):
    ...
    def print_info(self):
        print(
            "  Position is (%d, %d); direction is %s"
            % (self.posy, self.posx, dir_name[self.dir])
        )
        if self.ip in self.labels_inv:
            print(
                "  Instruction pointer is %d, labelled with %s"
                % (self.ip, ", ".join(self.labels_inv[self.ip]))
            )
        else:
            print("  Instruction pointer is %d, without labels" % (self.ip))
    ...
    def execute_step(self):
        ...
        if (
            ...
            and self.step_num == self.max_steps
            and not halting
        ):
            halting = True
            self.killed = True
    ...
    for line in fin:
        ...
        try:
            ...
            if posx < 0 or posx >= len(grid):
                raise ParserException(
                    "Column index %d out of bound " "in line %s" % (posx, line)
                )
            ...
            if num < 0 or num >= 16:
                raise ParserException(
                    "Pebble number %d out of bound " "in line %s" % (num, line)
                )
        except ValueError:
